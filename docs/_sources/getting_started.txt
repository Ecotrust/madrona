...
Introduction
************
Madrona is a framework for building web-based spatial decision support tools. 
...
If your problem involves spatial data and multi-user collaboration, Madrona is the premier tool. 
...
.. note::
    Madrona development tends to follow django trunk. It may work on the 
...
#. The following python packages listed in `madrona_requirements.txt <http://madrona.googlecode.com/hg/madrona_requirements.txt>`_::  

.. literalinclude:: ../madrona_requirements.txt
...
We have created the `pip requirements file <http://madrona.googlecode.com/hg/madrona_requirements.txt>`_ to automate the installation of most of the dependencies::

    cd /usr/local # Assuming you want to put stuff in /usr/local/src
    pip install -r http://madrona.googlecode.com/hg/madrona_requirements.txt    
