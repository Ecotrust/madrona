...
class ClipToShapeManipulator(ManipulatorBase):
    '''
        ...
        return:
            a dictionary containing a 'message', 'html', the 'clipped_shape', and the 'orginal_shape'
        ...
        html_templates==9   if either "target_shape" or "clip_against" was not found in kwargs
                            or if geometry can not be generated from "clip_against" 
    ...
    def manipulate(self):
        ...
        if ret_shape.area == 0:
            ...
            return {"message": "intersection resulted in empty geometry", "html": status_html, "clipped_shape": ret_shape, "original_shape": target_shape}
        ...
        return {"message": "'target_shape' was clipped successfully to 'clip_against'", "html": status_html, "clipped_shape": largest_poly, "original_shape": target_shape}
    ''' 
...
class ClipToStudyRegionManipulator(ManipulatorBase):
    '''
        ...
        returns:
            a dictionary containing 'message', 'html', 'clipped_shape', 'orginal_shape'
            all of the returned shape geometries will be in srid GEOMETRY_CLIENT_SRID (4326) 
        ...
        html_templates==9   if "target_shape" was not found in kwargs
                            or if Study Region not found in database
                            or intersection call failed
                            clipped_shape will be returned as None
        html_templates==3   if geometry can not be generated from "target_shape" 
                            or if "target_shape" is not a valid geometry
    ...
    def manipulate(self):                    
        ...
        if clipped_shape.area == 0:
            status_html = render_to_string(self.Options.html_templates["2"], {'MEDIA_URL':settings.MEDIA_URL})
            return {"message": "clipped geometry is empty (no overlap with study region?)", "html": status_html, "clipped_shape": clipped_shape, "original_shape": target_shape}
        
        largest_poly = LargestPolyFromMulti(clipped_shape)
        
        status_html = render_to_string(self.Options.html_templates["0"], {'MEDIA_URL':settings.MEDIA_URL})
        return {"message": "'target_shape' was clipped to study region", "html": status_html, "clipped_shape": largest_poly, "original_shape": target_shape}
...
class ClipToGraticuleManipulator(ManipulatorBase):
    ...
    def manipulate(self):
        ...
        if clipped_shape.area == 0:
            status_html = render_to_string(self.Options.html_templates["2"], {'MEDIA_URL':settings.MEDIA_URL})
            return {"message": "Graticule dimensions produce an empty clipped geometry", "html": status_html, "clipped_shape": clipped_shape, "original_shape": target_shape}
