...
class Mpa(models.Model):
    ...
    class QuerySet(GeoQuerySet):
        def add_kml(self):
            ...
            """
            return self.extra(select={'kml':'replace(AsKML(ST_Reverse(ST_ForceRHR(ST_Translate(ST_Force_3d("%s"."geometry_final"), 0, 0, 100)))), \'<Polygon>\', \'<Polygon><altitudeMode>absolute</altitudeMode><extrude>1</extrude>\')' % str(self.model._meta.db_table)})
