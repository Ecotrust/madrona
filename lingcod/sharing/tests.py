...
class SharingTestCase(TestCase):
    def setUp(self):
        ...
        mpa1 = Mpa.objects.create( name='Test_MPA_1', designation='R', user=self.user1, geometry_final=g1)
        mpa1.save()
        self.mpa1_id = mpa1.id

        mpa2 = Mpa.objects.create( name='Test_MPA_2', designation='R', user=self.user2, geometry_final=g1)
        mpa2.save()
        self.mpa2_id = mpa2.id

        mpa3 = Mpa.objects.create( name='Test_MPA_3', designation='R', user=self.user3, geometry_final=g1)
        mpa3.save()
    ...
    def test_nothing_shared(self):
        ...
        """
        shared_mpas = Mpa.objects.shared_with_user(self.user1)
    ...
    def test_share_mpa(self):
        ...
        mpa2 = Mpa.objects.get(id=self.mpa2_id)
        ...
        shared_mpas = Mpa.objects.shared_with_user(self.user3)
        self.assertEquals(len(shared_mpas),0)
    ...
    def test_share_mpa_shortcut(self):
        ...
        viewable, response = can_user_view(Mpa, self.mpa2_id, self.user2)
        self.assertEquals( viewable, True )
        # User1 should see it (since they're part of Group1)
        viewable, response = can_user_view(Mpa, self.mpa2_id, self.user1)
        self.assertEquals( viewable, True )
        # User3 should not see it since they're not part of Group1
    ...
    def test_share_by_bad_user(self):
        ...
        mpa3 = Mpa.objects.get(id=self.mpa3_id)
        group1 = Group.objects.get(id=self.group1_id)
        error_occured = False
        try:
...
#        array1 = Array.objects.get(id=self.array1_id)
#        group1 = Group.objects.get(id=self.group1_id)
#        share_object_with_group(array1, group1) 
#        # User2 should see it (since they're part of Group1)
#        shared_mpas = Array.objects.shared_with_user(self.user2)
#        self.assertEquals(len(shared_mpas),1)
#        # User3 should not see it (since they're not part of Group1)
#        shared_mpas = Array.objects.shared_with_user(self.user3)
#        self.assertEquals(len(shared_mpas),0)
#
...
#        shared_mpas = Mpa.objects.shared_with_user(self.user2)
#        self.assertEquals(len(shared_mpas),1)
#        # User3 should not see it (since they're not part of Group1)
#        shared_mpas = Mpa.objects.shared_with_user(self.user3)
#        self.assertEquals(len(shared_mpas),0)
...
#        viewable, response = can_user_view(Mpa, self.mpa1_id, self.user1)
#        self.assertEquals( viewable, True )
#        # User2 should see the mpa contained in array1 (since they're part of Group1)
#        viewable, response = can_user_view(Mpa, self.mpa1_id, self.user2)
#        self.assertEquals( viewable, True )
#        # User3 should not see it (since they're not part of Group1)
#        viewable, response = can_user_view(Mpa, self.mpa1_id, self.user3)
#        self.assertEquals( viewable, False )
    ...
    def test_groups_users_sharing_with(self):
        ...
        mpa1 = Mpa.objects.get(id=self.mpa1_id)
        group1 = Group.objects.get(id=self.group1_id)
        share_object_with_group(mpa1, group1) 
