...
z={enableSelection:function(){return false},visitFunction:function(d,e){return e},openNetworkLinks:true,refreshWithState:true,bustCache:false,restoreState:false,supportItemIcon:false,loadingMsg:"Loading data"};return function(d){var e={},l=0,p={};e.lookupTable=p;e.kmlObject=null;var q={},F={};d=jQuery.extend({},z,d);var i=d.gex.pluginInstance,J=false;if(parseFloat(i.getPluginVersion())<5.1)throw"kmltree requires a google earth plugin version >= 5.1";if(!d.url||!d.gex||!d.element)throw"kmltree requires options url, gex & element";
...
i.getFeatures().appendChild(a);c=new n({success:function(){D();$(e).trigger("kmlLoaded",[a])},error:function(){D();$(e).trigger("kmlLoadError",[a])},tree:e,my:F});if(!e.previousState)if(d.restoreState&&window.localStorage)e.previousState=W();e.previousState&&e.previousState.children.length?B(d.element.find("div.kmltree"),e.previousState,c):P(c,$("#"+d.element.attr("id")));c.execute()}else if(l===0){l++;setTimeout(function(){jQuery.ajax({url:b,success:function(){e.load(true)},error:function(){L(a,
b)}})},100)}else setTimeout(function(){d.element.html(['<div class="kmltree"><h4 class="kmltree-title">Error Loading</h4><p class="error">could not load kml file. Try clicking <a target="_blank" href="',b,'">this link</a>, then refreshing the application.<p></div>'].join(""));$(e).trigger("kmlLoadError",[a])},0)},P=function(a,b){b.find("li.KmlNetworkLink.open").each(function(){if(!$(this).hasClass("checkHideChildren")){var c=$(this);c.removeClass("open");a.add(c,function(f){f.addClass("open");r(f,
"open",c.hasClass("open"));P(a,f);a.execute()})}})},O=function(a){var b=X(a),c={name:a.getName()};google.earth.executeBatch(i,function(){d.gex.dom.walk({visitCallback:function(f){var g=f.current;if(!g.children)g.children=[];var h=Y(b,this),k=this.getSnippet();if(!k||k==="empty")k=false;h={renderOptions:E,name:this.getName()||"&nbsp;",visible:!!this.getVisibility(),type:this.getType(),open:this.getOpen(),id:this.getId().replace(/\//g,"-"),description:this.getDescription(),snippet:k,select:d.enableSelection(this),
listItemType:Z(this),customIcon:aa(this),customClass:"",children:[],kmlId:h,alwaysRenderNodes:false};h=d.visitFunction(this,h);g.children.push(h);if(h.listItemType!=="checkHideChildren")f.child=h;else f.walkChildren=false},rootObject:a,rootContext:c})});return c},aa=function(a){if(!d.supportItemIcon)return false;return(a=kmldom(a.getKml()).find("kml>Folder, kml>Document, kml>Placemark, kml>NetworkLink").find(">Style>ListStyle>ItemIcon>href").text())?a:false},Z=function(a){var b="check";if((a=a.getStyleSelector())&&
a.getType()==="KmlStyle")if(a=a.getListStyle())switch(a.getListItemType()){case 0:break;case 5:b="radioFolder";break;case 2:b="checkOffOnly";break;case 3:b="checkHideChildren";break}return b},E=function(a){if(jQuery.isArray(a)){for(var b=[],c=0;c<a.length;c++)b.push(Q(a[c]));return b.join("")}else return Q(a)},ba={renderOptions:E},Q=function(a){return o(jQuery.extend({},ba,a))},R=function(){p=null;p={}};e.refresh=function(){if(d.refreshWithState)e.previousState=G();R();S();T();i.setBalloon(null);
M(true)};var T=function(){for(var a in q){var b=q[a];b&&b.getParentNode()&&d.gex.dom.removeObject(b)}q={}},S=function(){T();e.kmlObject&&e.kmlObject.getParentNode()&&d.gex.dom.removeObject(e.kmlObject);e.kmlObject=null},W=function(){var a=localStorage.getItem("kmltree-("+d.url+")");return a?JSON.parse(a):false},ca=function(){var a=JSON.stringify(G());localStorage.setItem("kmltree-("+d.url+")",a)};e.destroy=function(){J=true;d.restoreState&&window.localStorage&&ca();R();S();var a=d.element.attr("id");
$("#"+a+" li > span.name").die();$("#"+a+" li").die();$("#"+a+" li > .expander").die();d.element.html("")};var m=function(a){a=$(a);if(a.length&&a.find("> .kmlId").length){var b=$($(a)[0]).find("> .kmlId").text().split("-");a=b[0];b=parseInt(b[1]);return p[a][b]}else return false};e.lookup=m;var Y=function(a,b){p[a].push(b);return a+"-"+(p[a].length-1)},U=function(a){a=$(a);return a.length&&a.find("> .nlDocId").length?q[a.find("> .nlDocId").text()]:false},X=function(a){var b=a.getName()+(new Date).getTime();
if(p[b])throw"document already added to lookup";q[b]=a;p[b]=[];return b},x=function(a,b){b||(b=m(a));a=$(a);t(true,true);a.addClass("selected");w(a,true);a.addClass("selected");openBalloon(b,i);for(var c=a.parent().parent();!c.hasClass("kmltree")&&!c.find(">ul:visible").length;){c.addClass("open");c=c.parent().parent()}r(a,"selected",true);$(e).trigger("select",[a,b])};e.selectNode=x;var y=function(a,b){if(b){if(!a.hasClass("checkOffOnly"))if(a.hasClass("radioFolder")){if(!a.find(">ul>li.visible").length){a=
