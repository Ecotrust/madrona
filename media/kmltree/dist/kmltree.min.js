...
1&&k.find(".kmltree-cursor-2").length===0&&f[l].instance.clearSelection()}};that._clearEverythingButMe=function(l){for(var k=0;k<f.length;k++)if(f[k].instance!==l)if($(f[k].api.opts.element).find(".kmltree-selected").length||$(f[k].api.opts.element).find(".kmltree-breadcrumb").length)f[k].instance.clearSelection()};var r=function(l,k){if(!l)return false;if(l.getUrl()===k)return true;if(l.getElementByUrl(k))return true},g=function(l){l=l.getUrl();var k=l.split("#")[0];if(e[k])return e[k];for(var m=
0;m<f.length;m++)if(r(f[m].instance.kmlObject,l)){e[k]=f[m];return f[m]}for(m=0;m<f.length;m++)for(var C=f[m].api.docs,u=0;u<C.length;u++)if(r(C[u],l)){e[k]=f[m];return f[m]}return false};that.getOwner=function(l){return(l=g(l))?l.instance:false};var v=function(l,k){$(window).unbind("message.kmlTreeIframeEvents");var m;k=k.instance?k.instance:k;var C=k.api?k.api:c(k);m=C.opts.displayEnhancedContent;if(typeof m==="function")m=m(l);if(m){var u=l.getBalloonHtmlUnsafe(),w=l.getBalloonHtml();w=$.trim(w.replace(/\s*<\!--\s*Content-type: mhtml-die-die-die\s*--\>/,
